<template>
	<view>
		<view class="view-first" v-for="(item,index) in teamList" :key="index">
			...
			<view class="view-user child" v-for="(child,childIndex) in item.popularizeTeamVoList" :key="childIndex">
	...
	export default {
		...
		methods: {
			async getTeamList() {
				...
				let res = await this.http.get('/app/userHome/directPushFanList', {
					pageSize: 10,
