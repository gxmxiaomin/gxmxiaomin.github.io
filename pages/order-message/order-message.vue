<template>
	<view>
		<view class="view-list" v-if="sysInfo.length != 0">
			<view class="view-item" v-for="(item,index) in sysInfo" @click="uni.navigateTo({
				url:'../my-order-details/my-order-details?id='+item.orderId
			})">
				<view class="view-top">
				...
				<view class="view-center">
					<image :src="item.goodsCoverImg?item.goodsCoverImg.split(',')[0]:''" mode=""></image>
