<template>
	<view>
		<u-navbar safeAreaInsetTop placeholder title="交易额提成" @leftClick="uni.navigateBack({
...
<script>
	export default {
		...
		methods: {
			confirm(e) {
				this.time =  new Date(e.value).format("yyyy-MM-dd HH:mm:ss")
				this.show = false
				this.getList()
			},
			async getList(){
				this.loadmoreStatus='loading';
				let res=await this.http.post("/app/userPersonalCenter/transactionAmountNumList",{
					pageNum:1,
					pageSize:10,
					startDate:this.time,
				});
