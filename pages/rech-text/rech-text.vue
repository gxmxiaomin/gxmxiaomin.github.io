	...
	<view>
		<view class="top-bar" :style="setH()">
			...
			<view class="bar-right bare-cell">
				...
				<text class="nextStep" @click="submit">下一步</text>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			...
			getdefImage(arr) { //获取富文本中第一个图片
				if (!Array.isArray(arr)) return ""
				try {
					arr.forEach(ele => {
						if (ele.insert && ele.insert.image) {
							throw new Error(ele.insert.image)
						}
					})
				} catch (e) {
			...
			submit() {
				...
				console.log(this.form.tags) //同城
				// console.log(this.fileList.length) //1
				this.editorCtx.getContents({
					success: async res => {
						console.log(res)
						console.log(this.getdefImage(res.delta.ops))
						this.defImg = this.getdefImage(res.delta.ops)
						this.form.type = 2
						...
						if (this.title != '') {
							...
							uni.navigateTo({
								url: './rech-text2?title=' + this.title + '&html=' + this.content +
									'&img=' + this.defImg
							})
						} else {
