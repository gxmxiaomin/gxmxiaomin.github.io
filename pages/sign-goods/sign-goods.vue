	...
	<view class="view-container">
			...
			<view class="view-desc">
				...
				<view class="view-rush-buy-price">
					<view class="view-price">
						...
						<text></text><text class="text-price">需要连续签到{{storeId.signNum}}天</text>
					</view>
				</view>
			</view>
		...
		<view class="view-bottom">
			<button class="u-reset-button exchange" @click="uni.navigateTo({
					url: '../submit-sign-order/submit-sign-order?data='+encodeURIComponent(JSON.stringify(storeId))
				})">立即领取</button>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		methods: {
			async clickScoreGoods() {
				uni.navigateTo({
					url: '../starCoinShop-order/starCoinShop-order?id=' + storeId.id
				})
				// let res = await this.http.postJson('/app/userScores/exchangeScoreGoods', {
				// 	scoreGoodsId:this.id,
				// 	goodsType:1,
				// 	userAddressId:1,
				// });
				// console.log('立即领取',res);
				// if (res.code == 200) {
