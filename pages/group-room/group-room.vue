	...
	<view class="chat-room-view">
		<TabBar color="#515151" backgroundColor="#fff" :isFill="true">
			...
			<view class="tab-right" slot="right">
				<u-icon name="setting" size="24" @click="uni.navigateTo({url:'/pages/group-info/group-info?groupId=' + outherInfo.username})"></u-icon>
		...
		<scroll-view scroll-y="true" :scroll-into-view="currentMsg" class="scroll-view" :scroll-top='scrollTop'
			...
			<view class="chat-lists">
				<view class="chat-line" v-for="(item, index) in chatList" :key="item.id" :id="'bottom-id' + item.id">
					...
					<!-- 对方发送的信息 -->
					<view class="chat-info-outher chat-line-info" v-if="item.from != String(myInfo.userName)">
						<image :src="getHaderUrl(item.from)" mode="" class="header-img" @click="goUserHome(item)"></image>
						<view class="message-content" style="margin-left: 20rpx;background-color: #fff">
					...
					<view class="chat-info-isme chat-line-info" v-else>
						...
						<view class="message-content" style="margin-right: 20rpx;">
							<view class="text-message" v-if="item.contentsType == 'TEXT'">
								<view v-for="(content,index) in decMessage(item.data)" :key="index">
									<text v-if="content.type=='text'"> {{ content.content }} </text>
									<image v-else :src="require('@/static/images/faces/' + content.content)" style="width: 20px;height: 20px;" mode=""></image>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			this.IM.readMessage(this.outherInfo.username, 'groupChat')
		...
		methods: {
			...
			getHaderUrl(user) {
				...
				if(a) return a.headUrl
				return "https://img1.baidu.com/it/u=3705123043,2281640962&fm=253&fmt=auto&app=138&f=JPEG?w=500&h=776"
			},

			async getGroupInfo(groupId) {
				// const res = await this.IM.getGroupInfo(groupId)
				// console.log('获取到的群聊的信息', res)
				// let info = res.data[0] || {}
				// this.getUserList( info.affiliations)
				const res = await this.http.get("/app/appGroup/getGroupInfo", {
					groupId: this.outherInfo.username
				})

			},

			async getUserList() { //获取群聊成员信息
			...
			getEmo(content,emoStyle ,list=[]) { //获取文本中的表情数组
			...
			decMessage(content) { //格式化文本加表情的信息
			...
			newMessage(msg) { //接收到新的消息
				console.log("这里时群聊监听接口我接受到了一条数据", msg)
				if(msg.to == this.outherInfo.username) {
					this.IM.readMessage(this.outherInfo.username, 'groupChat')
					this.chatList.push(msg)
					this.scollToBottom()
				} else {
					this.$refs.messageTip.showTip({
						message:"您有一条新消息"
					})
				}
			},
			...
			async emojiAction(e) {
				...
				console.log(e.msg)
				this.inputText = this.inputText + e.msg
			},
			...
			async getHistoryMessage() { // 获取历史信息
				...
				const res = await this.IM.messageRoam(this.outherInfo.username, true)
			...
			sendImg() {
				uni.chooseImage({
					...
					success: async (res) => {
						...
						const result = await this.IM.sendPrivateUrlImg(file, 'groupChat')
						this.sendSuccess()
			...
			addImgMessage(url) { //发送图片
				let data = {
					contentsType: "IMAGE",
					url: url,
					from: String(this.myInfo.userName),
			...
			async sendMessage() {
				...
				let res = await this.IM.sendGroupText(data)
				this.sendSuccess()
				this.addTextMessage()
				this.inputText = ""
				console.log("发送成功")
