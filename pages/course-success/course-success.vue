...
<script>
	export default {
		...
		onLoad(option) {
			...
			this.discipline = option.discipline
			this.traineeGrade = option.traineeGrade
			this.traineeName = option.traineeName

			this.phone = option.phone
			this.city = option.city
			this.category = option.category
			this.traineeSex = option.traineeSex
			this.traineeAge = option.traineeAge
			this.traineeCarId = option.traineeCarId
		},
		methods: {
			async confirm(e) {
			...
			async postPay() {
				...
				let res = await this.http.post('/app/userHomeNavigation/payTuition', {
					rechargeType: this.rechargeType,
					schoolId: this.schoolId,
					totalPrice: this.total,
					payType: this.payType,
					discipline: this.discipline,
					className: this.traineeGrade,
					traineeName: this.traineeName,
					traineeSex: this.traineeSex,
					traineeAge: this.traineeAge,
					traineeCarId: this.traineeCarId
				});
				if (res.code == 200) {
					...
					uni.requestPayment({
						provider: this.rechargeType,
						orderInfo: this.rechargeType == 'wxpay' ? JSON.parse(res.data) : res
