<template>
	<view class="view-container">
		...
		<view class="view-bottom">
			<button class="u-reset-button" @click="submit">提交资料</button>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		onLoad(options) {
			this.form.employmentPositionId = options.id
		},
		onShow() {
			this.getFileUrl();
		},
		methods: {
			async submit() {
				...
				let res = await this.http.postJson('/app/employmentPosition/signUp', this.form);
				if (res.code == 200) {
					this.form = {
			...
			// async getSignUp() {
			// 	let res = await this.http.post('/app/employmentPosition/signUp', {
			// 		id: this.ids
			// 	});
			// 	if (res.code == 200) {
			// 		this.$toast(res.msg);
