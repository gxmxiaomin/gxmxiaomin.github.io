<template>
	<view>
		<view class="child-nav" :style="'top:'+statusBarHeight+'rpx;'">
			<image class="image-fanhui" src="../../../static/images/31fanhui1.png" @click="uni.navigateBack({})">
			</image>
		...
		<view class="view-switch">
			...
			<view class="view-content" v-if="qiehuan==1">
				<view class="view-introduction">
					<text class="text-title">擅长方向</text>
					<view class="view-details">
						<view class="view-label" v-for="(item,index) in introduce.arr" :key="index">
							{{item}}
						</view>
					</view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			to() {
				uni.navigateTo({
					url: '../student-message/student-message?id=' + this.val + '&schoolTypeId=' + this.introduce
						.schoolTypeId
				})
			},
			bofang() {
			...
			async getIntroduce() {
				...
				if (res.code == 200) {
					...
					let text = [];
					let goodAtDirection = res.data.schoolInfo.goodAtDirection;

					if (goodAtDirection) {
						let goodAtDirectionText = goodAtDirection.split(",");
						console.log(goodAtDirectionText);
