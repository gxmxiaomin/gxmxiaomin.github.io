		...
		<view class="view-list">
			<view class="view-item" v-for="(item,index) in list">
				...
				<view class="view-attention">
					<button type="default" @click="focus(item.focusId)" v-if="item.focusStatus==0&&item.status==0">+关注</button>
					<button type="default" @click="cancelFocus(item.focusId)" v-if="item.focusStatus==0&&item.status==1" class="button-cancel">取消关注</button>
					<button v-if="item.focusStatus==1&&item.status==1" 
					@click="cancelFocus()"
	...
	export default {
		...
		methods: {
			...
			async getList() {
				...
				let res = await this.http.get('/app/forum/user/selectFocusList', {
					pageNum: this.page,
