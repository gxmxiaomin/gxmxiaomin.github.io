<template>
    <view class="content" @touchmove.stop.prevent v-if="isShow" @click.stop="change">
...
<script>
    export default{
        props:{
			...
			copyWrite:{
				type:String,
				default:'邀请您参加本活动,可以扫描右边二维码,关注公众号咨询'
			},
            title:{
                type: String,
                default: '草莓千层蛋糕'
        ...
        data(){
            return {
                ...
                ctx: null,
                isShow: false,
                qrcode: 'http://www.muxingchen.com/static/image/muxingewm.02d2322.png'
            }
        },
        methods:{
			change(){
            ...
            /*     let headerImg = await this.getImageInfo(this.headerImg)
                ...
                this.ctx.setFillStyle('#858585');
                this.ctx.fillText(this.copyWrite.split(',')[0],(((this.canvasW-hW) / 2) + uni.upx2px(30)),(((this.canvasW-hW) / 2) + hH + uni.upx2px(220)))
                this.ctx.fillText(this.copyWrite.split(',')[1],(((this.canvasW-hW) / 2) + uni.upx2px(30)),(((this.canvasW-hW) / 2) + hH + uni.upx2px(270)))
                this.ctx.fillText(this.copyWrite.split(',')[2],(((this.canvasW-hW) / 2) + uni.upx2px(30)),(((this.canvasW-hW) / 2) + hH + uni.upx2px(320)))
                //底部广告
                ...
                // this.ctx.drawImage(BottomAdImg.path,(((this.canvasW-hW) / 2)),(((this.canvasW-hW) / 2) + hH + uni.upx2px(310)),uni.upx2px(350),uni.upx2px(110))
                 let BottomAdImg = await this.getImageInfo(this.qrcode)
                    ...
                    complete: (res) => {
                        ...
                        uni.saveImageToPhotosAlbum({
                            ...
                            success(res) {
                                ...
                                setTimeout(()=>{
                                    that.isShow = false
									that.$parent.$parent.uqrcode = false
                                },2000)
                            }
                        })
                    }
