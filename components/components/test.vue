<template>
	<view>
		<view class="box">
			...
			<view class="type-2">
				<text class="type-2-1" :style="9 == titleIndex?'background: #4b9bfd;color:#fff':''"
					@click="quanbu">全部</text>
			</view>
		...
		<view class="view-goods-container">
			<view class="view-goods-list" v-if="shopLsit.length > 0">
				<view class="view-goods-item" v-for="(item,index) in shopLsit" :key='item.id' @click="DownloadMethods(isShare,uni.navigateTo({
					url:'../interest-course-details/interest-course-details?id='+item.id
				}),invitationCode)">
					...
					<view class="view-goods-info">
						...
						<view class="text">
							<text>{{item.courseStartTime}}</text> <text
								style="margin-left: 12%;">{{item.signUpNum}}加入学习</text>
						</view>
						<view>
...
<script>
	export default {
		props: ['course', 'isShare' , 'invitationCode'],
		data() {
